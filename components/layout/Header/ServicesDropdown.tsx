...
const serviceToSlug = (serviceName: string): string => {
  const serviceSlugMap: { [key: string]: string } = {
    ...
    'Ajmal Perfumes': 'ajmal-perfumes',
    'Rasasi Perfumes': 'rasasi-perfumes',
    'Swiss Arabian': 'swiss-arabian',
    'Al Haramain Perfumes': 'al-haramain-perfumes'
  };
  
  return serviceSlugMap[serviceName] || serviceName.toLowerCase().replace(/\s+/g, '-').replace(/&/g, '').replace(/\./g, '');
};
...
const categoryToQuery = (categoryName: string): string => {
  const categoryMap: { [key: string]: string } = {
    ...
    'Perfume Shops': 'perfume',
    'Kiosks & Small Cabins': 'kiosk'
  };
...
const ServicesDropdown = ({ isOpen, onMouseEnter, onMouseLeave }: ServicesDropdownProps) => {
  ...
  const handleCategoryClick = (category: string) => {
    ...
    if (categoryQuery) {
      router.push(`/services-directory?category=${categoryQuery}`);
    } else {
  ...
  const handleServiceClick = (service: string) => {
    ...
    router.push(`/services-directory/${slug}`);
  };
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: '6px', md: '8px', lg: '10px' } }}>
            {servicesDropdownData.topPicks.map((service, index) => (
              <Box
                key={index}
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: '6px', md: '8px', lg: '10px' } }}>
            {servicesDropdownData.categories.map((category, index) => (
              <Box
                key={index}
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: '6px', md: '8px', lg: '10px' } }}>
            {servicesDropdownData.services.map((service, index) => (
              <Box
                key={index}
                component={Link}
        ...
        >
          ...
          >
            Discover essential services at Giga Mall including banking, grocery shopping, perfumes, and convenient kiosks...
          </Typography>
          <Box
            component={Link}
            href="/services-directory"
