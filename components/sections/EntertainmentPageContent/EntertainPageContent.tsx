...
import { EntertainDetail } from '@/lib/utils/entertainData';
import EntertainHero from '@/components/sections/EntertainHero/EntertainHero';
...
interface EntertainPageContentProps {
  entertain: EntertainDetail;
}

const EntertainPageContent = ({ entertain }: EntertainPageContentProps) => {
  // Default location info if not provided
  const locationInfo = entertain.location || {
    level: 'First Floor',
  ...
  return (
    ...
    >
      {/* Entertain Hero Section */}
      <EntertainHero entertain={entertain} />

      {/* Entertain Location Section - Map and Restaurant Details */}
      <Box
        sx={{
          width: '100%',
          backgroundColor: '#ffffff',
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Contact Information */}
            {entertain.contact && (entertain.contact.phone || entertain.contact.email) && (
              <Box
                sx={{
                  display: 'flex',
                  flexDirection: 'column',
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 1.5, sm: 1.75, md: 2, lg: 2 } }}>
                  {entertain.contact.phone && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 1.25, md: 1.5, lg: 1.5 } }}>
                      ...
                      <Typography
                        component="a"
                        href={`tel:${entertain.contact.phone}`}
                        sx={{
                          fontFamily: '"Quicksand", sans-serif',
                          fontSize: { xs: '13px', sm: '14px', md: '15px', lg: '16px' },
                          color: '#333333',
                  ...
                  {entertain.contact.email && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 1.25, md: 1.5, lg: 1.5 } }}>
                      ...
                      <Typography
                        component="a"
                        href={`mailto:${entertain.contact.email}`}
                        sx={{
                          fontFamily: '"Quicksand", sans-serif',
                          fontSize: { xs: '13px', sm: '14px', md: '15px', lg: '16px' },
                          color: '#333333',
...
export default EntertainPageContent;
