...
import { ServiceDetail } from '@/lib/utils/servicesData';
...
// Helper to convert ServiceDetail to StoreDetail for StoreHero compatibility
const serviceToStoreDetail = (service: ServiceDetail): StoreDetail => {
  return {
    name: service.name,
    slug: service.slug,
...
const ServicePageContent = ({ service }: ServicePageContentProps) => {
  // Default location info if not provided
  const locationInfo = service.location || {
    level: 'First Floor',
    parking: 'P2, Grand Parking'
  };

  // Convert service to store format for StoreHero
  const storeDetail = serviceToStoreDetail(service);
  ...
  return (
    ...
    >
      ...
      <StoreHero store={storeDetail} />

      {/* Service Location Section - Map and Service Details */}
      <Box
        sx={{
          width: '100%',
          backgroundColor: '#ffffff',
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Contact Information */}
            {service.contact && (service.contact.phone || service.contact.email) && (
              <Box
                sx={{
                  display: 'flex',
                  flexDirection: 'column',
