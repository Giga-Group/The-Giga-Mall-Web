...
interface DineHeroProps {
  entertain: DineDetail;
}
//Test
// Helper function to get logo path from entertain slug - matches StoreGrid logic
const getEntertainLogo = (slug?: string): string | null => {
  if (!slug) return null;
...
const DineHero = ({ entertain }: DineHeroProps) => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('lg'));
  
  // Get mobile or desktop image based on screen size
  const backgroundImageSrc = entertain.backgroundImage 
    ? (isMobile ? getMobileDineImage(entertain.backgroundImage, entertain.name, entertain.slug) : entertain.backgroundImage)
    : null;
  
  // Get logo from StoreGrid mapping, fallback to entertain.logo if available
  let logoPath = getEntertainLogo(entertain.slug) || entertain.logo || null;
